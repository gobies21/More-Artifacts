...
public class WitheredBezoarItem extends Item implements ICurioItem {
    ...
    public void onMobEffectApplicable(MobEffectEvent.Applicable event) {
        if (event.getEntity() instanceof Player player) {
            if (event.getEffectInstance() != null && event.getEffectInstance().getEffect() == MobEffects.POISON ||
                ...
                CuriosApi.getCuriosHelper().findEquippedCurio(ModItems.WitheredBezoar.get(), player).ifPresent((slot) -> {
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        pTooltipComponents.add(Component.literal("§7Grants immunity to Poison and Wither"));
