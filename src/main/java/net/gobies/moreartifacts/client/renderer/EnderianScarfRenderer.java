...
import net.gobies.moreartifacts.client.models.EnderianScarfModel;
...
public class EnderianScarfRenderer implements ICurioRenderer {
    private static final ResourceLocation TEXTURE = new ResourceLocation("moreartifacts", "textures/models/enderian_scarf.png");
    private final EnderianScarfModel<LivingEntity> model;

    public EnderianScarfRenderer() {
        this.model = new EnderianScarfModel<>(Minecraft.getInstance().getEntityModels().bakeLayer(MAModelLayer.ENDERIAN_SCARF));
    }
    ...
    public <T extends LivingEntity, M extends EntityModel<T>> void render(ItemStack stack, SlotContext slotContext, PoseStack poseStack, RenderLayerParent<T, M> renderLayerParent, MultiBufferSource bufferSource, int light, float limbSwing, float limbSwingAmount, float partialTicks, float tickDelta, float headYaw, float headPitch) {
        ...
        this.model.enderian_scarf.render(poseStack, vertexConsumer, light, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
        poseStack.popPose();
